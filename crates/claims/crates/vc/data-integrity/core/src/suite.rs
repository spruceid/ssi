...
use iref::Iri;
use linked_data::{to_quads, LinkedDataPredicateObjects, LinkedDataResource, LinkedDataSubject};
use rdf_types::{
    generator, interpretation, ExportedFromVocabulary, InterpretationMut, Literal, Quad,
...
use crate::{
    sign, signing, ExpandedConfiguration, ExpandedConfigurationRef, Proof, ProofConfiguration,
    ProofConfigurationCastError, ProofConfigurationRef, ProofConfigurationRefExpansion,
...
pub trait CryptographicSuiteInput<T, C = ()>: CryptographicSuite {
    ...
    ) -> Result<Verifiable<Claims<T, Proof<Self>>>, signing::Error<C::LoadError>>
    where
