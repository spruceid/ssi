...
use ssi_vc_data_integrity_core::{
    ...
    CryptographicSuiteInput, ExpandedConfiguration, ExpandedConfigurationRef,
...
where
    ...
    L: ssi_vc_data_integrity_suites::eip712::TypesProvider,
{
        ...
        macro_rules! ld_crypto_suites {
            {
                ...
                match self {
                    $(
                        ...
                        Self::$name => {
							match params.try_cast_verification_method() {
								Ok(params) => {
									Ok(ssi_vc_data_integrity_suites::$name.transform(
										data,
										&mut context.ld,
										params
									).await?.into())
					...
					Self::TezosJcsSignature2021 => {
						match params.try_cast_verification_method() {
							Ok(params) => {
								Ok(ssi_vc_data_integrity_suites::TezosJcsSignature2021.transform(
									data,
									&mut (),
									params
								).await?.into())
					...
					Self::EthereumEip712Signature2021 => {
						match params.try_cast_verification_method() {
							Ok(params) => {
								Ok(ssi_vc_data_integrity_suites::EthereumEip712Signature2021.transform(
									data,
									&mut context.loader,
									params
								).await?.into())
					...
					Self::EthereumEip712Signature2021v0_1 => {
						match params.try_cast_verification_method() {
							Ok(params) => {
								Ok(ssi_vc_data_integrity_suites::EthereumEip712Signature2021v0_1.transform(
									data,
									&mut context.loader,
									params
								).await?.into())
