    ...
    fn prepare_message<'b>(&self, bytes: &'b [u8]) -> Cow<'b, [u8]> {
        match self {
            ...
            Self::TezosWallet => ssi_crypto::SignatureProtocol::<AnyBlake2b>::prepare_message(
                &ssi_vc_data_integrity_suites::tezos::TezosWallet,
                bytes,
            ),
        }
    }
...
impl From<ssi_vc_data_integrity_suites::tezos::TezosWallet> for AnySignatureProtocol {
    fn from(_value: ssi_vc_data_integrity_suites::tezos::TezosWallet) -> Self {
        Self::TezosWallet
    }
}
