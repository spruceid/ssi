...
use ssi_vc_data_integrity_suites::{AnySignature, AnySignatureRef};
...
pub struct UnsupportedProofSuite(pub ssi_vc_data_integrity_core::Type);
...
macro_rules! crypto_suites {
    {
        ...
        impl TryFrom<ssi_vc_data_integrity_core::Type> for AnySuite {
            type Error = UnsupportedProofSuite;

            fn try_from(
                ty: ssi_vc_data_integrity_core::Type
            ) -> Result<Self, Self::Error> {
                $(
                    $(#[cfg($($t)*)])?
                    {
                        let suite = ssi_vc_data_integrity_suites::$name;
            ...
            I: ReverseIriInterpretation<Iri = V::Iri>
        {
                ...
                >
            {
                ...
                for i in interpretation.iris_of(resource) {
                    ...
                    $(
                        $(#[cfg($($t)*)])?
                        if iri == ssi_vc_data_integrity_suites::$name::IRI {
                            return Ok(Self::$name)
                        }
                    )*
                }
                    ...
                    Some(iri) => {
                        Err(linked_data::FromLinkedDataError::UnsupportedIri {
                            ...
                            supported: Some(vec![
                                $(
                                    $(#[cfg($($t)*)])?
                                    ssi_vc_data_integrity_suites::$name::IRI.to_owned(),
                                )*
                            ])
                        })
                    }
        ...
        pub enum AnySignatureAlgorithm {
            $(
                $(#[cfg($($t)*)])?
                $name(<ssi_vc_data_integrity_suites::$name as ssi_vc_data_integrity_core::CryptographicSuite>::SignatureAlgorithm)
            ),*
        }
        ...
        //     $(
        //         $(#[cfg($($t)*)])?
        //         $name(#[pin] SuiteSign<'a, ssi_vc_data_integrity_suites::$name, SignerAdapter<S, ssi_jwk::Algorithm, AnySignatureProtocol>>)
        //     ),*
        // }
        ...
        impl CryptographicSuite for AnySuite {
            ...
            fn name(&self) -> &str {
                match self {
                    $(
                        $(#[cfg($($t)*)])?
                        Self::$name => ssi_vc_data_integrity_suites::$name.name()
                    ),*
                }
            }
            ...
            fn iri(&self) -> &iref::Iri {
                match self {
                    $(
                        $(#[cfg($($t)*)])?
                        Self::$name => ssi_vc_data_integrity_suites::$name.iri()
                    ),*
                }
            }
            ...
            fn cryptographic_suite(&self) -> Option<&str> {
                match self {
                    $(
                        $(#[cfg($($t)*)])?
                        Self::$name => ssi_vc_data_integrity_suites::$name.cryptographic_suite()
                    ),*
                }
            }
            ...
            fn hash(&self, data: Transformed, proof_configuration: ExpandedConfiguration<Self::VerificationMethod, Self::Options>) -> Result<Self::Hashed, HashError> {
                match self {
                    $(
                        $(#[cfg($($t)*)])?
                        Self::$name => {
                            ssi_vc_data_integrity_suites::$name.hash(
                                data.try_into()?,
                                proof_configuration
            ...
            fn setup_signature_algorithm(&self) -> Self::SignatureAlgorithm {
                match self {
                    $(
                        $(#[cfg($($t)*)])?
                        Self::$name => AnySignatureAlgorithm::$name(ssi_vc_data_integrity_suites::$name.setup_signature_algorithm())
                    ),*
                }
            }
        }
