...
macro_rules! impl_rdf_input_urdna2015 {
    ($ty:ident) => {
        impl<V: rdf_types::Vocabulary, I: rdf_types::Interpretation, E, T>
            $crate::ssi_vc_data_integrity_core::CryptographicSuiteInput<T, E> for $ty
        where
            ...
            T::Expanded: linked_data::LinkedData<I, V>,
        {
            ...
            async fn transform<'t, 'c: 't>(
                ...
                data: &'t T,
                context: &'t mut E,
                _options: $crate::ssi_vc_data_integrity_core::ExpandedConfigurationRef<
                    'c,
                    <$ty as $crate::ssi_vc_data_integrity_core::CryptographicSuite>::VerificationMethod,
                    <$ty as $crate::ssi_vc_data_integrity_core::CryptographicSuite>::Options,
                >,
            ) -> Result<Self::Transformed, $crate::ssi_vc_data_integrity_core::suite::TransformError> {
                let expanded = data
                    .expand(context)
                    .await
                    .map_err(|_| $crate::ssi_vc_data_integrity_core::suite::TransformError::ExpansionFailed)?;
