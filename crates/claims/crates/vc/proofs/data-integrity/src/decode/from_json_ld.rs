    ...
    L::Error: Send,
{
    ...
    let (mut remote, expanded) = document
    ...
    match remote.document_mut().as_object_mut() {
        Some(object) => {
    ...
    match expanded.into_main_node() {
        Some(mut node) => match vocabulary.get(PROOF_IRI).map(json_ld::Id::iri) {
            Some(proof_iri) => {
                ...
                {
                    Some(proof_object) => match proof_object.into_inner() {
                        json_ld::Object::Node(proof_node) => {
                            let document =
                                json_ld::Document::new(remote, json_ld::Indexed::none(node).into());
