use serde::Serialize;
use ssi_claims_core::{ClaimsValidity, Options, ValidateClaims};
...
impl<T> ValidateJwsHeader for JWTClaims<T> {
    fn validate_jws_header(&self, _env: &Options, _header: &ssi_jws::Header) -> ClaimsValidity {
        Ok(())
    }
}

impl<P, T: ClaimSet + ValidateClaims<P>> ValidateClaims<P> for JWTClaims<T> {
    fn validate_claims(&self, env: &Options, proof: &P) -> ClaimsValidity {
