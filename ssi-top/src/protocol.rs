    ...
    fn encode_signature(&self, algorithm: ssi_jwk::Algorithm, signature: Vec<u8>) -> Result<Vec<u8>, MessageSignatureError> {
        match self {
            ...
            Self::TezosWallet => {
                ...
                ssi_vc_ldp::suite::tezos::TezosWallet.encode_signature(algorithm, signature)
            }
        }
    }
...
impl From<ssi_vc_ldp::suite::tezos::TezosWallet> for AnySignatureProtocol {
    fn from(_value: ssi_vc_ldp::suite::tezos::TezosWallet) -> Self {
        Self::TezosWallet
    }
}
