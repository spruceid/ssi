...
use rdf_types::{
    ...
    IriVocabularyMut, LanguageTagVocabularyMut, LiteralInterpretationMut, LiteralVocabularyMut,
...
{
    #[pin]
    extract: LendingMutFuture<
        super::from_json_ld::ExtractJsonLdProof<'a, I, V, L>,
        (&'a mut V, &'a mut I),
    >,
    t: PhantomData<T>,
}
...
{
    type Output = Result<
        (T, json_ld::Node<V::Iri, V::BlankId>),
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> task::Poll<Self::Output> {
        ...
        this.extract
            .poll(cx)
            .map(|(r, (vocabulary, interpretation))| {
                let (json_ld, proof) = r?;
