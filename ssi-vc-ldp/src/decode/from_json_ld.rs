...
{
    #[pin]
    expand: LendingMutFuture<BoxFuture<'a, IntoDocumentResult<V::Iri, V::BlankId, (), L>>, &'a mut V>,
    interpretation: &'a mut I
}
    ...
    pub(crate) fn new(
        ...
        Self {
            expand: LendingMutFuture::new(vocabulary, |vocabulary| {
                document.into_document_with_using(
                    vocabulary,
                    loader,
                    options
                )
                                ...
                                Some(Stripped(Meta(proof_object, _))) => {
                                    match proof_object.into_inner() {
                                        json_ld::Object::Node(proof_node) => {
                                            ...
                                            eprintln!("proof node: {}", (*proof_node).with(&*vocabulary).pretty_print());
