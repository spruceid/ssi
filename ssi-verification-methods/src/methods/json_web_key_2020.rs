...
use crate::{
    LinkedDataVerificationMethod, VerificationMethod, CONTROLLER_IRI, RDF_JSON, RDF_TYPE_IRI,
};
...
pub struct JsonWebKey2020 {
    ...
    /// Key controller.
    controller: IriBuf, // TODO: should be an URI.
...
impl VerificationMethod for JsonWebKey2020 {
    ...
    async fn verify(
        ...
        match self.public_key.algorithm.as_ref() {
            Some(a) => {
                Ok(ssi_jws::verify_bytes(*a, signing_bytes, &self.public_key, signature).is_ok())
            }
